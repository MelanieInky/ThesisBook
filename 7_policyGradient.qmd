...
Once we know a specific state, that is the problem parameters, we need to choose the two solver parameters $\Delta t$ and $\alpha$. A specific action is then a pair $a = (\Delta t, \alpha) \in \mathbb{R^+}\times [0,1]$. The policy is then denoted by $\pi(a = (\Delta t, \alpha) |s = (b,n))$.
...
Both $p(s',r|s,a)$ and the reward $r$ are not dependent on the policy, and therefore $\theta$. The gradient is thus
...
$$
\nabla_{\mathbfit{\theta}} J(\theta) \propto E\left[\sum_a q_\pi(a,S)\nabla \pi(a|S, {\mathbfit{\theta}})\right],
$$
 
where $S$ is the random variable with probability mass function $\mu(s)$.
...
where $n$ is the number of samples we have. Using the gradient ascent algorithm in @eq-gradient_ascent_algorithm, we can update the parameters $\theta$:
...
| **INPUT:** 
| - A parameter vector $\theta \in\mathbb{R}^d$, and a parametrized policy $\pi(a|s,\theta)$ with computable gradient $\nabla_\theta \pi(a|s,\theta)$.
| - Learning rate $\alpha$
| - Discount rate $\gamma$
| - Episode length $\tau+1$
...
|       Compute an episode, following $\pi(a|s,\theta)$, of length T+2 the form  $S_0,A_0 \to S_1,A_1,R_1 \to S_2,A_2,R_2 \to \dots \to S_{\tau+1}, R_{\tau+1}$;
|       **FOR** t=0 ... \tau:
...
Because of the finite episode length. The REINFORCE algorithm is more suited for episodic tasks, but it is also usable for continuing tasks, if we accept some bias. Another alternative would be to discard the last few estimated returns $\hat{G}_\tau, \hat{G}_{\tau-1}, \dots$ as they are the most biased. 
:::
