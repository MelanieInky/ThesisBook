...
Let $\lambda_1 , \lambda_2 , \dots , \lambda_n$ be the (not necessarily distinct) eigenvalues of $A$. We write $\lambda_i = x_i + iy_i$, where $x_i,y_i \in \mathbb{R}$, and are respectively the real part and the imaginary parts of the eigenvalue. Then following holds:
...
We assume that $A\in\mathbb{R}^{n\times n}$ is diagonalizable. Let $\lambda_1,\dots,\lambda_n$ be the eigenvalues of $A$. Then we can write $A = P\Delta P^{-1}$ where $\Delta$ is the diagonal matrix with the eigenvalues of $A$, and $P$ is the associated eigenvectors matrix.  
...
Then $e^{At} = \sum_{i=0}^\infty \frac{(P\Delta P^{-1}t)^i}{i!} = \sum_{i=0}^\infty P \frac{(\Delta t)^i}{i!}P^{-1}$. The $P$ can be moved outside of the sum to get 

$$
e^{At} = P e^{\Delta t}P^{-1}.
$$
