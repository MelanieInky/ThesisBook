...
where $y_0\in \mathbb{R}^n$ and $t\in \mathbb{R}$. We adapt the result below from [@ODE_book, chap. 9.5]. 
...
Let $\lambda_1 , \lambda_2 , \dots , \lambda_n$ be the (not necessarily distinct) eigenvalues of $A$. We write $\lambda_i = x_i + iy_i$, where $x_i,y_i \in \mathbb{R}$ and are respectively the real part and the imaginary parts of the $i^{th}$ eigenvalue. Then, the following results holds[@BellmanStab, chap. 1]:
