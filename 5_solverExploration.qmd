...
The goal is for the solver to converge to a steady state solution as fast as possible. 
...
In the test problem case, $f(t_n,\mathbfit{u_n}) = \mathbfit{e} - \mathbfit{Mu_n}$, and we get the update

$$
\mathbfit{u_{n+1}} = \mathbfit{u_n} + \Delta t\left[\mathbfit{e}-\mathbfit{M}(\mathbfit{u}_n+\alpha\Delta t(\mathbfit{e}-\mathbfit{Mu_n}))\right].
$$

After a few lines of computation, we get the following iteration:

$$
\mathbfit{u}_{n+1} = \left[\mathbfit{I}- \Delta t(\mathbfit{I}- \alpha \Delta t \mathbfit{M})\mathbfit{M}\right]\mathbfit{u_n} + \left[\Delta t(\mathbfit{I} - \alpha \Delta t \mathbfit{M})\right]\mathbfit{e}.
$${#eq-stationary_linear_iter}


This iteration is of the form $\mathbfit{u_{n+1}} =\mathbfit{Ku_n}  + \mathbfit{Le}$, where 
...
We recognize this iteration as a linear stationary iteration, which converges to $\mathbfit{u}^* = \mathbfit{M}^{-1}\mathbfit{e}$ if and only if $\rho(K)<1$, [@iterative_method_book, chap. 2.2], where $\rho(\mathbfit{K})$ is the spectral radius of $\mathbfit{K}$. 

:::{.remark}
One may remark that $\mathbfit{K} = p(\Delta t \mathbfit{M})$ with $p(z) = 1- \alpha z + z^2$ a polynomial. This polynomial is also the stability polynomial of the RK method [@BirkenNotes].
:::

### Residual ratios

We set $\mathbfit{u_0} = \mathbfit{u(}0\mathbfit{)} = \mathbfit{e}$ as an initial value. We define the relative residual after $k$ steps as 

$$
r_k = \frac{||\mathbfit{M}u_k - \mathbfit{e}||}{||\mathbfit{e}||},
...
$$
\rho_k = \frac{||r_k||}{||r_{k-1}||} = \frac{||\mathbfit{M}u_k - \mathbfit{e}||}{||\mathbfit{M}u_{k-1}-\mathbfit{e}||}.
...
As we've seen in @fig-residual_ratio_evolution, $\rho_i$ can vary quite a bit depending on $i$, so we decide to investigate the residual ratio after 10 iterations and 100 iterations. We set the problem parameters $b = 0.05$, and $n = 100$, and we plot $\rho_{k}= f_k(\Delta t, \alpha)$ for different values of $k$. This is achieved by making a linear grid for parameters $\Delta t$ and $\alpha$ of size $100\times 100$, where $\alpha$ varies between $0$ and $1$, and $\Delta t$ varies between $0$ and $5$, then computing the residual ratios on that grid. 
