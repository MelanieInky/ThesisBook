...
## Stability theory for numerical solver
...
$$
u_{n+1} = u_n + \Delta t\left[e-M(u_n+\alpha\Delta t(e-Mu_n))\right].
$$

After a few lines of computation, we get the following iteration:

$$
u_{n+1} = \left[I- \Delta t(I- \alpha \Delta t M)M\right]u_n + \left[\Delta t(I - \alpha \Delta t M)\right]e.
...
$$
L = \Delta t\left[ I - \alpha \Delta t M\right]
$${#eq-L}
and 
$$
K = I- LM = \left[I- \Delta t(I- \alpha \Delta t M)M\right].
$${#eq-K}


We recognize this iteration as a linear stationary iteration, which converges to $u^* = M^{-1}e$ if and only if $\rho(K)<1$, [@iterative_method_book, chap. 2.2], where $\rho(K)$ is the spectral radius of $K$. 
