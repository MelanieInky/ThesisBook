...
Numerical methods for differential equations are amongst the most important and numerous methods in numerical analysis, all with specific strengths and weaknesses. They all have, however, some parameters that need to be chosen, if only for the step size.

These parameters have to be chosen to maximize performances, and depends on the problem. In some cases they are taken using some heuristics, but they can also be searched for computationally. 
