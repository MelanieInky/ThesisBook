...
We apply a policy gradient method to our problem

### Objective function.

Now that we control the policy via the parameter vector $\theta$, we ought to determine a way to find the best value for $\theta$. We define the objective function
