...
- The discount rate is set to a low value of $\gamma = 0$, as the state transitions are completely random, there is no reason to prefer long term rewards. This also mean that the approximated return in the REINFORCE algorithm is unbiaised!
...
The slow apparent convergence rate can not be mitigated by a higher learning rate, as this empirically lead to divergences issues. 
...
Consider the objective function $f(x,y) = x^2 + 9y^2$. The function admits a global minimum at $x = y = 0$, and its gradient is 
...
That is, $x_{t+1} = (1-2\alpha_x)x_t$ and $z_{t+1} = (1-2\alpha_y)y_t$. This converges to $0$ if and only if $0<\alpha<\frac{1}{2}$, which means we can affort a much bigger learning rate. With $\alpha = \frac{1}{2}$, the gradient descent algorithm can now converge to $0$ in a single iteration! 
...
which can vary between $0$ and $1$. Everything then follows by simply replacing $n$ by $n'$ in @sec-linear_policy. The new deterministic policy is 
...
an expression of the gradient is unchanged, with the exception of replacing $n$ by $n'$ everywhere. 

With this change implemented, we rerun the first experiment. All the parameters are the same, except that the learning rate can is now set to $\alpha = 3\times 10^{-4}$ without divergence. Compared to the first experiment, the evolution of the policy parameters is much higher! The average episode reward is also much better.

While the average reward converges. The same cannot be said for the $\theta$ parameters. In particular, TODO seem to never converge. Running the program for more episode did not resolve this issue.  

The results are in @fig-variable_theta.
...
initial_theta_p1 = '[0.5,0.5,0.5,0.5,0.5,0.5]'
initial_theta_p2 = '[0,0,0,0,0,0]'
...
p1 = (ggplot(df_exp_tidy, aes(x = 'episode_no_1000', y = 'value', colour = 'Initial theta'))
+ geom_line()
+ facet_wrap('~variable', scales='free_y')
+ theme_seaborn()
...
#| echo: false
#| warning: false
#| fig-cap: Evolution of the rolling average($k=200$) of the average episode reward for different initial parameters. 
#| label: fig-reward_initial_theta

#Don't use the tidy data 
df_exp_reward = df_exp.select('ep_number', 'avg_ep_reward','Initial theta').with_columns(pl.col('avg_ep_reward').rolling_mean(window_size=200).alias('rolling reward'))
...
Gradient descent algorithms use the local information about an objective function $J(\theta)$ to compute the update $\theta \rightarrow \theta + \alpha \nabla J(\theta)$. This local behavior also means that any convergence of gradient descent is to a local minimum, but we can't be certain that this mimimum is a global mimimum.

 Therefore, it is interesting to test whether the algorithm converges to the same values regardless of initial conditions. The third experiment is then to run the algorithm with the same parameters as before, but with varied initial conditions for the $\theta$ vector, and to visualize the results, both in the average rewards and the evolution of $\theta$ parameters over 100000 episodes. 

 The evolution of the $\theta$ parameters is in @fig-variable_theta, and the rolling average of the average episode reward is plotted in @fig-reward_initial_theta for different initial value for $\theta$. It turns out that while convergence in reward is to the same values, the convergence of the $\theta$ parameters is more difficult to observe.  
...
$$
\theta^* = [4.02 \times 10^{-4},1.09\times 10^{-2},-6.36\times 10^{-2},-0.335 ,0.238,1.11]
$$
...
### Setting the policy
problem.policy.theta = np.array([4.02e-4,1.09e-2,-6.36e-2,-3.35e-01 ,0.238,1.11])
