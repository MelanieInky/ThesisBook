...
$$
r(c_{10}) = \begin{cases}
s\times (1-c_{10}) \;\;\;\;\;\qquad \text{ if } c_{10}<1\\
\max(-3,1 - c_{10}) \qquad \text{ if } c_{10}\geq1
\end{cases}
$$

When $c_{10}<1$, the reward is positive as we are currently converging, and the lower the ratio, the better the convergence and thus we want a better reward. The term $s$ is there t. 
...
where $\alpha_{\text{var}} = (\alpha_{0}, \alpha_{1}, \dots , \alpha_{5})^\intercal$ and $\odot$ represent the termwise vector multiplication. Based on this, we set 

$$
\alpha_{\text{var}} = (\alpha_0,\alpha_1, \dots, \alpha_5)^\intercal = 2\times 10^{-8}(70 , 0.3,50 , 0.3,70 ,50 )^\intercal.
$$

This learning rate is set partly based on the justification above, and is also found experimentally through trial and error so as to accelerate down the learning of the parameters not "dependent" on $n$. The results are in @fig-variable_learning_theta.
...
Compared to the last thing, we get a much faster reward
...
df_exp_reward = df_exp.select('ep_number', 'avg_ep_reward','experiment_number').with_columns(pl.col('avg_ep_reward').rolling_mean(window_size=1000).alias('rolling reward'))
...
In the algorithm that we used before, a single 



## Impact of initial condition.

Gradient descent algorithm use the local information about a loss function $l(\theta)$ to compute the update $\theta \rightarrow \theta + \alpha \nabla l(\theta)$. When gradient descent converges, we can only be sure that the convergence is to a local minimum, but we can't be certain that this mimimum is a global mimimum. Therefore, it is interesting to test whether the algorithm converge  
