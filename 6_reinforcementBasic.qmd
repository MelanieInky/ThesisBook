...
:::{.remark}
Bellman's equation is dependent on the given policy and gives a recursive relation for the state values. Solving this equation is called policy evaluation which involves fixed point iterations(see example below). 
:::
...
We stop here to remark that this equation is of the form $v_\pi = r_\pi + \gamma P_\pi v_\pi$. $P_\pi$ is row stochastic. Furthermore, since $\gamma<1$, we motivate solving the equation by using fixed point iterations. This is the main idea behind *dynamic programming* @bellman1957dynamic. In this case, we can simply solve the system directly. With $\gamma= 0.5$, we get the state values $v_\pi(s_1) = 7.875$, $v_\pi(s_2) = 4.625$. There is, for the given policy, more value in being in the first state than the second. 
...
|       **FOR** all state $s$:
|                Compute, for all $a\in\mathcal{A}$,the action values $q_{\pi_i}(a,s)$;
